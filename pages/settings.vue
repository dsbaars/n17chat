        ...
        <div class="card-body">
          ...
          <div class="flex flex-wrap gap-2 my-2">
            <div class="badge badge-outline gap-1 py-3">
              <StatusIndicator status="connected" size="xs" />
              <span class="ml-1">{{ relayStore.dmRelays.filter(r => relayStore.isConnected(r)).length }} Connected</span>
            </div>
            <div class="badge badge-outline gap-1 py-3">
              <StatusIndicator status="disconnected" size="xs" />
              <span class="ml-1">{{ relayStore.dmRelays.filter(r => !relayStore.isConnected(r)).length }} Disconnected</span>
            </div>
          </div>
          ...
          <ul class="space-y-2">
            <li v-for="relay in relayStore.dmRelays" :key="relay" class="flex items-center justify-between p-2 bg-base-100 rounded-lg">
              <div class="flex items-center space-x-2 min-w-0">
                <StatusIndicator 
                  :status="relayStore.isConnected(relay) ? 'connected' : 'disconnected'" 
                  size="xs"
                />
